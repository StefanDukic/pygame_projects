...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if(self.rect.y >= WIN_HEIGHT - self.radius or self.rect.y <= 0):
            try:
                o1.previous_ball_hit_list.remove(self)
            except:
                pass
...
class Game:
    ...
    def play(self):
        ...
        balls_list = [
            Ball([0.25 * WIN_WIDTH, 0.75 * WIN_HEIGHT], [-1, 0], 50, 10, BLUE)
            , Ball([0.25 * WIN_WIDTH, 0.50 * WIN_HEIGHT], [-1, 0], 100, 10, BLACK)
            , Ball([0.50 * WIN_WIDTH, 0.75 * WIN_HEIGHT], [-1, 0], 100, 100, WHITE)
            , Ball([0.50 * WIN_WIDTH, 0.25 * WIN_HEIGHT], [-1, 0], 100, 10, RED)
        ]
        ...
        while True:
            ...
            for k in Balls:
                self.ball_hit_list = pygame.sprite.spritecollide(k, Balls, False, pygame.sprite.collide_circle_ratio(0.55))


                if len(self.ball_hit_list) > 1:
                    if self.ball_hit_list == self.previous_ball_hit_list:

                        pass
                    else:
                        Ball.collide(self, self.ball_hit_list)



                    self.previous_ball_hit_list = self.ball_hit_list
